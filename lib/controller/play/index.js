...
class PlayController {
    ...
    _getAutoplayVideoId() {
        ...
        if (lastMixPlaylist && lastMixPlaylist.items.length > lastMixPlaylist.currentIndex + 1) {
            ...
            getMixPlaylistInfo = VideoHelper.getMixPlaylistInfo(self.lastPlaybackInfo.videoId, lastMixPlaylist.session)
        }
        else {
            getMixPlaylistInfo = VideoHelper.getMixPlaylistInfo(self.lastPlaybackInfo.videoId);
        }
        getMixPlaylistInfo.then( (mixPlaylist) => {
            if (mixPlaylist && mixPlaylist.items.length > mixPlaylist.currentIndex + 1) {
                ...
                yt2.getLogger().info(`[youtube2-play] _getAutoplayVideoId(): resolving to next video in Mix Playlist (video Id: ${nextVideoId} ... Mix playlist Id: ${mixPlaylist.id} ... index: ${mixPlaylist.currentIndex})`);
