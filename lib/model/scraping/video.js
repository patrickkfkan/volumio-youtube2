...
class VideoModel extends ScraperBaseModel {
    ...
    _doGetVideo(videoId) {
        ...
        ytdl.getBasicInfo(videoId).then( (result) => {
            ...
            let thumbnail = info.thumbnail && info.thumbnail.thumbnails ? info.thumbnail.thumbnails.pop().url : '/albumart';
            let video = new Video(info.videoId, info.title, thumbnail, new Channel(info.externalChannelId, info.ownerChannelName));
            defer.resolve(video);
        }).catch( (error) => {
            defer.reject(error);
        });
    ...
    getNextPageTokenFromFetchResult(result, options) {
        if (options.search) {  // return from youtube-scrape response
            if (result.key && result.nextPageToken) {
    ...
    convertToEntity(item, options) {
        if (options.search) {  // convert from youtube-scrape response
            ...
            return new Video(video.id, video.title, video.thumbnail_src, new Channel(null, uploader.username));
        }
        else if (options.playlistId) {  // convert from ytpl response
            return new Video(item.id, item.title, item.bestThumbnail.url, new Channel(item.author.channelID, item.author.name));
        }
        return null;
    }
