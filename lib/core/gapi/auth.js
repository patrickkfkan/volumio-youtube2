...
class AuthManager {
    ...
    constructor(credentials, logger) {
        ...
        if (credentials) {
            self.getAccessTokenFromStorage().then( (accessToken) => {
    ...
    setAndSaveAccessTokenToStorage(accessToken) {
        ...
        let accessTokenObj = accessToken || {};
        fs.writeFile(yt2PluginLibRoot + '/../storage/access_token', JSON.stringify(accessTokenObj), (error) => {
            if (error) {
                this.logger.error('[youtube2-auth] Failed to save access token: ' + JSON.stringify(error));
            }
        });
    }
    ...
    getAccessTokenFromStorage() {
        ...
        fs.readFile(yt2PluginLibRoot + '/../storage/access_token', 'utf8', (error, data) => {
            if (error) {
                this.logger.error('[youtube2-auth] getAccessTokenFromStorage(): Error reading file');
                ...
                try {
                    ...
                    this.logger.error('[youtube2-auth] getAccessTokenFromStorage(): Parse error');
                    this.logger.error(e);
                    parsed = null;
                }
                ...
                this.logger.error('[youtube2-auth] getAccessTokenFromStorage(): Unknown error');
                defer.resolve(null);
            }
        });
