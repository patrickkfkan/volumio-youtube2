...
class PlaylistFetcher {
  ...
  static async doGetChannelPlaylist(channelId, tryChannelType = 'defaultOrTopic', sort) {
  ...
  static async getChannelPlaylistLast (channelId) {
    return await this.getChannelPlaylist(channelId, 'lad');
  }
  ...
  static async parseChannelPlaylistResponse (response, tryChannelType) {
    ...
    if (tryChannelType === 'defaultOrTopic' || tryChannelType === 'topic') {
      ...
      const tryGetFromTab = (tabIndex) => {
        try {
          return response.data[1].response.contents.twoColumnBrowseResultsRenderer.tabs[tabIndex].tabRenderer.content.sectionListRenderer.contents[0].itemSectionRenderer.contents[0].shelfRenderer.content.gridRenderer;
        } catch (e) {
          return null
        }
      }
      ...
      // Playlists could be under fourth or fifth tab, depending on
      // whether there's a preceding Explore tab
      playlistData = tryGetFromTab(3) || tryGetFromTab(4)
    }
    ...
    if (typeof continuationItem !== 'undefined' && continuationItem.length > 0) {
      continuation = JSON.stringify({
        token: continuationItem[0].continuationItemRenderer.continuationEndpoint.continuationCommand.token,
